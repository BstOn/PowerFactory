...
public class ShowTrainResult {
	 ...
	 */
	public ShowTrainResult(TrainPlanDTO trainPlanInfoDTO, List<TrainPlanDTO> allTrainPlanItems, String id) {
	...
	private void initialize(final TrainPlanDTO trainPlanInfoDTO, List<TrainPlanDTO> allTrainPlanItems,
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String remark1 = "已提交";
				trainp.Is_submit(remark1, id);
				JOptionPane.showMessageDialog(null, "成绩已提交！");
				trainp.IsFinish("已培训", id);
			}
		});
		btnNewButton.setBounds(337, 380, 97, 23);
		...
		for (int i = 0; i < allTrainPlanItems.size(); i++) {
			...
			remark = trainPlanInfoDTO.getRemark1();
			// 成绩是否提交
			if (remark.equals("已提交")) {
				remark = "查看成绩";
			} else {
				remark = "录入成绩";
			}
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				if (SwingUtilities.isLeftMouseButton(arg0) && arg0.getClickCount() == 2) {
					...
					if (table.getSelectedRowCount() == 1) {
						...
						// 作为参数初始化修改界面
						TrainAddResult trainAndEmp = new TrainAddResult(Item_id, id);
						trainAndEmp.main(null);
					} else
